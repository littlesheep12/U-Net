...
def getArgs():
    ...
    parse.add_argument("--action", type=str,
                       help="train/test/train&test", default="train&test") #选择运行模式
    parse.add_argument("--epoch", type=int, default=100)
    parse.add_argument('--arch', '-a', metavar='ARCH', default='UNet',
                       help='UNet/resnet34_unet/unet++/myChannelUnet/Attention_UNet/segnet/r2unet/fcn32s/fcn8s')  # 算法中选择一个
    parse.add_argument("--batch_size", type=int, default=8)
    parse.add_argument('--dataset', default='kaggleLung',
                       help='dataset name:liver/esophagus/dsb2018Cell/corneal/driveEye/isbiCell/kaggleLung')  # 分割类型中选择一个
...
def getLog(args):
    dirname = os.path.join(args.log_dir, args.arch, str(
        args.batch_size), str(args.dataset), str(args.epoch))
...
def val(model, best_iou, val_dataloaders):
    ...
    with torch.no_grad():
        ...
        if aver_iou > best_iou:
            ...
            torch.save(model.state_dict(), r'./saved_model/'+str(args.arch)+'_' +
                       str(args.batch_size)+'_'+str(args.dataset)+'_'+str(args.epoch)+'.pth')
...
def train(model, criterion, optimizer, train_dataloader, val_dataloader, args):
    ...
    metrics_plot(args,'iou',iou_list) #单独iou
    metrics_plot(args,'dice',dice_list) #单独dice
...
def test(val_dataloaders, save_predict=False):
    ...
    if save_predict == True:
        dir = os.path.join(r'./saved_predict', str(args.arch),
                           str(args.batch_size), str(args.epoch), str(args.dataset))
        if not os.path.exists(dir):
            os.makedirs(dir)
        else:
            print('dir already exist!')
    model.load_state_dict(torch.load(r'./saved_model/'+str(args.arch)+'_'+str(args.batch_size) +
                          '_'+str(args.dataset)+'_'+str(args.epoch)+'.pth', map_location='cpu'))  # 载入训练好的模型
    model.eval()
...
if __name__ == "__main__":
    ...
    if 'train' in args.action:
        train(model, criterion, optimizer,
              train_dataloaders, val_dataloaders, args)
